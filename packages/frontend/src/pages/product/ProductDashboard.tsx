import React, {useEffect} from 'react'
import { Link, Outlet, useNavigate } from "react-router-dom";
import type { AuthState } from "../../services/auth/types";
import { RootState, useAppSelector } from '../../store';
import { useLogoutMutation } from '../../services/auth/authSlice';

export const ProductDashboard = ({
	isAuthenticated,
	authState,
}: { isAuthenticated: boolean; authState: AuthState }) => {
    const cart = useAppSelector((state:RootState)=>state.cart.products );
	const [logout, { isLoading }] = useLogoutMutation();
	const navigate = useNavigate()

    useEffect(() => {
        if(!isAuthenticated){
            navigate("/",{replace:true})
        }
    }, [])
  return (
    <div class=" p-6 ">
        <>
        	{isAuthenticated ? (
            	<div class=" flex justify-between items-center py-6">
                    <img
							className=" h-10 w-auto"
							src="https://tailwindui.com/plus/img/logos/mark.svg?color=indigo&shade=600"
							alt="Your Company"
						/>
						<h2 className=" text-center space-x-8 text-2xl font-bold leading-9 tracking-tight text-gray-900 flex items-center">
							{authState.user.username}
                            <Link to={encodeURI(`cart`)}>
                            <div class=" relative flex items-start">
                               
                               <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#5f6368"><path d="M280-80q-33 0-56.5-23.5T200-160q0-33 23.5-56.5T280-240q33 0 56.5 23.5T360-160q0 33-23.5 56.5T280-80Zm400 0q-33 0-56.5-23.5T600-160q0-33 23.5-56.5T680-240q33 0 56.5 23.5T760-160q0 33-23.5 56.5T680-80ZM246-720l96 200h280l110-200H246Zm-38-80h590q23 0 35 20.5t1 41.5L692-482q-11 20-29.5 31T622-440H324l-44 80h480v80H280q-45 0-68-39.5t-2-78.5l54-98-144-304H40v-80h130l38 80Zm134 280h280-280Z"/></svg>
                               <span class=" rounded-full border text-white font-bold -mx-2 leading-3 text-[10px] bg-blue-600 px-1">{cart.length}</span>
                               </div>
                	</Link>
					<button onClick={()=> {
						try {
							logout().then(()=> {
								navigate(`/`, {
									replace: true,
								})
								
							})
						} catch (error) {
							
						}
					}} className="px-4 py-1 rounded border font-bold text-sm">Logout</button>
						</h2>

						
                </div>
        	) : (
            	<Link to={"/"}>back to home</Link>
        	)}
        	{authState.user.role === "seller" && (
            	<h4>
                	You can create products {" "}
                	<Link to={encodeURI(`create/${authState.user?.id}`)}>
                    	here
                	</Link>{" "}
                	too!
            	</h4>
        	)}
        	<Outlet />
    	</>
    </div>
  )
}
